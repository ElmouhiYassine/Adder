...
def U_pass(U: np.ndarray,
    ...
    def resolve_carry(c_segment: list[int], start_bit: int) -> tuple[int, int]:
        ...
        if not has_zero:
            val = 0
            for bit_pos, trit in enumerate(c_segment, start=start_bit):
                if trit == +1:
                    val |= (1 << bit_pos)
        ...
        if carry_policy == "definite":
            ...
            val = 0
            for bit_pos, trit in enumerate(c_segment, start=start_bit):
                if trit == +1 or trit == 0:  # treat 0 as +1
                    val |= (1 << bit_pos)
            return val, 0

        elif carry_policy == "min":
            # 0 -> -1 (i.e., contributes 0)
            val = 0
            for bit_pos, trit in enumerate(c_segment, start=start_bit):
                if trit == +1:
                    val |= (1 << bit_pos)
            ...
            # Expected value: -1->0, +1->1, 0->0.5; round to nearest int
            val_float = 0.0
            for bit_pos, trit in enumerate(c_segment, start=start_bit):
    ...
    for i in range(out_H):
        for j in range(out_W):
            ...
            if all_certain:
                ...
                c_segment = acc[K1:U_EXT_WIDTH-1]
...
def convolution(X: np.ndarray,
    ...
    U_trunc, carry_vals, carry_mask = U_pass(U, kernel, ripple_add)
...
def phi_center(t):
    ...
    return 0.0 if t == -1 else (0.0 if t == 0 else 1.0)
...
def run_mnist_noise_experiment(n_images: int = 32,
    ...
    for idx in range(imgs.shape[0]):
        ...
        Y = convolution(X, kernel, SK_ripple_add, balanced_ternary_add, K1=K1, K2=K2)
...
if __name__ == "__main__":
    ...
    noise_levels = [5, 10, 15, 20, 25, 30, 31]
