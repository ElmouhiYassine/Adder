...
for level in range(N_BITS + 1):
    ...
    V = len(vecs)
    print(f"Level {level}: {V} vectors, unordered pairs = {V*(V+1)//2}")

    # accumulators
    acc = {name: {"min": [], "max": [], "mid": []} for name in adders}

    # iterate unordered pairs WITHOUT repetition (A,B) with combinations_with_replacement
    for A, B in itertools.combinations_with_replacement(vecs, 2):
        A = list(A); B = list(B)
...
with open("uncertainty_results_vectors_4_bits.json", "w") as f:
    json.dump(results, f, indent=2)

print("Saved uncertainty_results_vectors 4.json")